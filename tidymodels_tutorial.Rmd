...
# big picture: take all the ones in fold = 1 and then take fold 2 through fold 10 to train the model, then go back to the fold = 1 and see how well the model (that was trained on fold = 2 through 10) does on the data it hasn't seen before, so on fold = 1
...
diamonds_validate <- diamonds_10fold %>%
  filter(fold == 1) # creating a data set for the fold that equals to 1
diamonds_train <- diamonds_10fold %>%
  filter(fold != 1) # creating a data set for the folds not equal to 10 (so 2 through 10)
  
### train on training partition // dataset, saving test data for validation
...
### use model to predict values for test dataset // test/validate on the validation partition
...
calc_rmse(test_df$resid1) ### 1467 (average error)
...
# how many times did it predict that they survived and how many actually survived; how many times did it predict they didn't survive and how many actually didn't survive
...
# correctly predicted survival 52 times
# incorrectly predicted survival for passengers who didn't survive 17 times
```
...
# about 81% accuracy, about 19% false positives/false negatives
```

![from https://glassboxmedicine.com/2019/02/23/measuring-performance-auc-auroc/](img/roc-curve-v2.png)
```{r}
# ROC = receiver operating characteristic [curve]
# area under this curve is a common metric referenced; tells us how good our model is at predicting
